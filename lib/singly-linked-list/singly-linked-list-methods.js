...
var assert = linkedListUtilities.assertions;
...
module.exports = {
     ...
     * 
     * @param {value} newhead 
     * set the head of the node.
     * throws an error if an invalid value is provided for the head node.
     * returns the head of the list if it was successfully set.
     */
    setHead: function (newhead) {
        var node = LinkedListNode.getValidNode(newhead);
        this.head = node;
        return this.head;
    },
    /**
    ...
    getLastNode: function () {
        ...
        return this.getNodeAt(currentSize);
    },
    /**
    ...
    shift: function () {
        ...
        var result = this.head.getValue();
        var newHead = this.head.getNext();
    ...
    pop: function () {
        ...
        var node = this.getNodeAt(currentSize - 1);
        node.setNext(null);
        return returnValue;
    },
    /**
    ...
    insertAt: function (position, value) {
        ...
        var previous = this.getNodeAt(position - 1);
    ...
    removeAt: function (position) {
        ...
        var tbd = this.getNodeAt(position);
        var prev = this.getNodeAt(position - 1);
        var next = this.getNodeAt(position + 1);
    ...
    reverse: function (from, to) {
        ...
        var startFrom = this.getNodeAt(_from);
        var counter = 0;
    ...
    createLoop: function (position1, position2) {
        ...
        this.getNodeAt(position2).setNext(this.getNodeAt(position1));
        return this;
    },
    /**
    ...
    isPalindrome: function () {
        ...
        var secondHalf = list.getNodeAt((currentSize % 2 === 0) ? mid + 1 : mid + 2);
        var counter = 0;
    ...
    findFromEnd: function (position) {
        assert.validPosition(this, position);
        assert.withinBounds(this, position);
        return this.getNodeAt((this.size() - position) + 1);
    },
    /**
    ...
    getPart: function (from, to) {
        ...
        var count = 0;
        var node = duplicate(this.getNodeAt(from));
    ...
    swap: function (position1, position2) {
        ...
        assert.validPosition(this, position2);
        assert.withinBounds(this, position2);
        var firstNode = this.getNodeAt(position1);
        var secondNode = this.getNodeAt(position2);
