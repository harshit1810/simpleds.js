 ...
 * @param {boolean} isAllowedToSetProperty defaults to true. provide false if you do not want to set property again;
 */
function makeGetterSetter(propertyName, propertyValue, validator, isAllowedToSetProperty) {
    var _isAllowedToSetProperty = (typeof isAllowedToSetProperty !== 'undefined' && isAllowedToSetProperty === false) ? false : true;
...
module.exports.SET_PROPERTY_ALLOWED = SET_PROPERTY_ALLOWED = true;
...
module.exports.isValidStackElement = function (element) {
    if (typeof element === 'undefined' || typeof element === 'symbol') {
        return false;
    }
    var invalidStrictEqualities = [Infinity, -Infinity, null];
    var isInvalid = false;
    for (var i in invalidStrictEqualities) {
        if (element === invalidStrictEqualities[i]) {
            isInvalid = true;
            break;
        }
    }
